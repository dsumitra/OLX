...
//			
	...
	void showUserSettings() {
		try {
		String query="Select email, first_name, last_name, address, phone, user_id from " + TableNames.USER + " where " + UserColumnNames.USER_ID + "=" + user.getId(); 
		ResultSet rs= DBConnection.executeQuery(query);
		int count=0;
		System.out.println("Email \t\t First Name \t Last Name \t  Address \t Phone");
		while(rs.next())
		 {
			  System.out.print(rs.getString("email")+ " \t " + rs.getString("first_name")+ " \t\t " + rs.getString("last_name")+ " \t\t " + rs.getString("address")+ " \t " +rs.getString("phone")+ " \n ");
		...
		System.out.println("1.Rename username \n 2.Change password \n 3.Update PhoneNumber \n 4.Update Email \n 5.Update Address");
